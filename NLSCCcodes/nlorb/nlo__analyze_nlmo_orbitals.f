         SUBROUTINE  NLO__ANALYZE_NLMO_ORBITALS
     +
     +                    ( NBAS,NATOM,
     +                      ATIDX,
     +                      BASBEG,BASEND,
     ...
     +
C------------------------------------------------------------------------
C  OPERATION   : NLO__ANALYZE_NLMO_ORBITALS
C  MODULE      : Natural Localized Orbitals
C  MODULE-ID   : NLO
C  DESCRIPTION : This routine analyzes the NLMO orbitals obtained.
C                An important quantity that is determined here is
C                the atomic localization, which measures the degree
C                of atomic orbital involvment in each NLMO. We can
C                extract the content of the correponding AOs in the
C                NLMO expansion via the following analysis:
C
C                In order to get rid of the AO nonorthogonality
C                problem, we express each NLMO in terms of the
C                orthonormal symmetrized AO basis (SAO), which
C                resembles most closely the original AO basis. The
C                new NLMO expansion coefficients c' in terms of SAOs
C                are then given by:
C
C                                 c' = S**(1/2) * c
C
C                where c is the coefficient vector for the NLMO in AO
C                basis. Suppose that we want to calculate the atomic
C                localization due to atom A from the NLMO. Then we form
C                the quantity:
C
C                                 x = c'(T)*c
C
...
C                corresponding to atom A in the NLMO.
C
C                The routine evaluates the complete atomic localization
C                map LOCAL (ATOM,NLMO) for each atom and NLMO.
C
C
C                  Input:
C
...
C                                    atom A.
C                    C            =  NBAS x NBAS NLMO coefficient matrix
C                                    in AO basis.
