         SUBROUTINE  MAT__DGEMM_CT_EQ_ATXB_FLOAT
     +
     +                    ( DDROWA,DDCOLA,
     +                      DDROWB,DDCOLB,
     +                      DDROWC,DDCOLC,
     ...
     +
C-----------------------------------------------------------------------
C  OPERATION   : MAT__DGEMM_CT_EQ_ATXB_FLOAT
C  MODULE      : MATRIX
C  MODULE-ID   : MAT
...
C                                C (T) = A (T) * B
C
C                The result will be stored in transposed form into C.
...
C
 ...
 1100          CONTINUE
               ...
               DO 3000 J = 1,COL,COLB
                  ...
                  DO 300 II = I,IIEND1,IDEPTH
                     II1 = II - I1
                     II2 = II - I2
                     DO 100 JJ = J,JJEND1,JDEPTH
                        JJ1 = JJ + 1
                        T11 = ZERO
                        T21 = ZERO
                        T12 = ZERO
  ...
  200                   CONTINUE
                        C(JJ,II) = C(JJ,II) + T11
                        C(JJ,II+1) = C(JJ,II+1) + T21
                        C(JJ1,II) = C(JJ1,II) + T12
                        C(JJ1,II+1) = C(JJ1,II+1) + T22
  100                CONTINUE
                     IF (JLEN.LT.JSPAN) THEN
                         DO 110 JJ = JJEND1+1,JJEND2
                            T11 = ZERO
                            T21 = ZERO
                            DO 210 KK = K,KKEND
                               KK1 = KK - K1
  ...
  300             CONTINUE
                  ...
                  IF (ILEN.LT.ISPAN) THEN
                      DO 320 II = IIEND1+1,IIEND2
                         II1 = II - I1
                         DO 120 JJ = J,JJEND1,JDEPTH
                            JJ1 = JJ + 1
                            T11 = ZERO
                            T12 = ZERO
                            DO 220 KK = K,KKEND
