...
C
C   IOPT YIELDS DIFFERENT OPTIONS. CURRENTLY ONLY THE CORE VARIANT HAS MORE OPTIONS
C    IOPT = 1:  DETERMINE SPECIFIC STATES OF INTEREST; MORE STRICT PROJECTION.
C    IOPT = 2:  DETERMINE A 'LOOSE' PATTERN, ALL STATES THAT MAY CONTRIBUTE SIGNIFICANTLY
...
C          
        write(6,*)
        WRITE(6,*) ' Core-orbitals included in excitation mask ', iopt
        write(6,*)
        DO ISPIN = 1, 1 + IUHF
          DO IRREP = 1, NIRREP
...
C
      ...
      if (print) then
       CALL PRMAINX(IUHF, SCR, MAXCOR, IRREPX)
      endif
C
      RETURN
      END
