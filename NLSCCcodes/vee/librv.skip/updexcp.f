      SUBROUTINE UPDEXCP(IUHF, SCR, DMAXCOR, IRREPX, IRREP, INDEX,
     &   PHTYPE,FACTOR, DOUBLE)
C
C  EXCITATION PATTERN IS UPDATED. IF ORBITAL CHARACTERIZED BY
C  'IRREP, INDEX(ISPIN), PHTYPE' IS PART OF EXCITATION COEFFICIENT, THE
C  EXCP-COEFFICIENTS ARE INCREASED BY FACTOR.
C  THE EXCP COEFFICIENTS ARE ASSUMED TO BE GIVEN ON LS2OUT AND LS1OUT
C
C
C                           all; each
        ...
        DO IIRREP = 1, NIRREP
          ...
          DO 1 I = 1, POP(IIRREP, SSPIN)
            DO 2 A = 1, VRT(AIRREP,SSPIN)
              IF (MATCH1(I,A,IIRREP, AIRREP,
     &           SSPIN,INDEX,IRREP,PHTYPE)) THEN
...
C
            ...
            DO 30 J= 1, POP(JIRREP,BSPIN)
              DO 40 I = 1, POP(IIRREP, ASPIN)
                DO 50 BIRREP = 1, NIRREP
                  ...
                  IF (MATCH2(I,J,A,B,IIRREP,JIRREP,AIRREP,BIRREP,
     &                     ASPIN,BSPIN,INDEX,IRREP,PHTYPE)) THEN
                    SCR(ICOUNT) = SCR(ICOUNT) + FACTOR
                  ENDIF
                    ICOUNT = ICOUNT + 1
   70             CONTINUE
   ...
   20   CONTINUE
        IF (ICOUNT .NE. I000 + NUMDSS*DISSYS) THEN
          WRITE(6,*)' SOMETHING WRONG IN UPDEXCP', ICASE,
