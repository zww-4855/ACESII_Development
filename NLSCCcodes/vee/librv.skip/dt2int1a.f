      SUBROUTINE DT2INT1A(ICORE,MAXCOR,IUHF,IRREPX,LISTW0,LISTT0,
     &                     LISTT1)
C
...
C CONTRACTION 1:
C
C     Z(A,I) = SUM  T(EF,IM) * <AM||EF> + T(Ef,Im) * <Am|Ef>  [AA]
C              mef
C
C     Z(a,i) = SUM  T(ef,im) * <am||ef> + T(eF,iM) * <aM|eF>  [BB]
C              mef
C
C     Z(A,I) = SUM  [2 T(Ef,Im) - T(Fe,IM)] <Am|Ef> [SPIN ADAPTED RHF] 
C              mef
CEND
...
C
C           T(Ef,Im) AND TRANSPOSE TO T(Efm,I)           [ISPIN=1]
C
C           T(FeM,i)                                     [ISPIN=2]
C
...
C                              +          
C         Z(A,I) = SUM W(Efm,A) * T(Efm,I)              [ISPIN=1]
C                  Efm
C                              +          
...
C ISPIN=1
C
C LOOP OVER m, READ IN ALL W(Ef,A) AND FORM Z(AI) <= W(Ef,A)*T(Efm,I)
...
C ISPIN=2
C
C LOOP OVER a, READ IN ALL W(Fe,M) AND FORM Z(ai) <= W(Fe,M)*T(Fe,Mi)
C FOR THIS a
C
          IDISW1=1
...
C
C     Z(A,I) = Z(A,I) + SUM  T(EF,IM) * <AM||EF>    [ISPIN=1]
C                       MEF
C
C     Z(a,i) = Z(a,i) + SUM  T(ef,im) * <am||ef>    [ISPIN=2]
C                       MEF
C
         ITTR=20+ISPIN
         ...
         I040=I030+IINTFP*3*MAXBUF
C
C READ T INTO T(E<F,M<I) AND EXPAND RHS TO T(E<F,MI)
C
         CALL GETLST(ICORE(I010),1,NUMDST,1,IRREPTR,LISTT)
...
C
C LOOP OVER M, READ IN ALL W(E<FA) AND FORM Z(AI) <= W(E<FA)*T(E<FM,I)
C FOR THIS VALUE OF M
C
          ITWR=8+ISPIN
          IDISW1=1
