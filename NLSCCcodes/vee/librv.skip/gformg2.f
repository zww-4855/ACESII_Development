      SUBROUTINE GFORMG2(IRREPC,IRREPT,LISTC0,LISTT0,LISTG,ICORE,
     &                  MAXCOR,IOF0,FACT,IUHF)
C
C THIS ROUTINE CARRIES OUT THE FOLLOWING TWO CONTRACTIONS :
C
C       G(BE) = SUM C(MN,BF) * T(MN,EF) + C(Mn,Bf) * T(Mn,Ef)   [UHF]
C       G(be) = SUM C(mn,bf) * T(mn,ef) + C(Nm,Fb) * T(Nm,Fe)   [UHF]
C       
C       G(MJ) = SUM C(EF,NJ) * T(EF,NM) + C(Fe,Jn) * T(Fe,Mn)   [UHF]
C       G(mj) = SUM C(ef,nj) * T(ef,nm) + C(Ef,Nj) * T(Ef,Nm)   [UHF]
C
C       G(be) = SUM C(Mn,Bf) * [2 T(Mn,Ef) - T(Nm,Ef)]          [RHF]
C
C       G(mj) = SUM C(Ef,Nj) * [2 T(Ef,Nm) - T(Fe,Nm)]          [RHF]
C
...
5      CONTINUE
C
       CALL SSCAL (LENGIJ,FACT,ICORE(I0GIJ),1)
       CALL SSCAL (LENGAB,FACT,ICORE(I0GAB),1)
       CALL GETLST(ICORE(I000),1,1,1,1+IOF0,LISTG+91)
       CALL SAXPY (LENGIJ,ONE,ICORE(I000),1,ICORE(I0GIJ),1)
       CALL PUTLST(ICORE(I0GIJ),1,1,1,1+IOF0,LISTG+91)
       CALL GETLST(ICORE(I000),1,1,1,1+IOF0,LISTG+92)
       CALL SAXPY (LENGAB,ONE,ICORE(I000),1,ICORE(I0GAB),1)
...
105     CONTINUE
        CALL SSCAL (LENGIJ,FACT,ICORE(I0GIJ),1)
        CALL SSCAL (LENGAB,FACT,ICORE(I0GAB),1)
        CALL GETLST(ICORE(I000),1,1,1,ISPIN+IOF0,LISTG+91)
        CALL SAXPY (LENGIJ,ONE,ICORE(I000),1,ICORE(I0GIJ),1)
        CALL PUTLST(ICORE(I0GIJ),1,1,1,ISPIN+IOF0,LISTG+91)
        CALL GETLST(ICORE(I000),1,1,1,ISPIN+IOF0,LISTG+92)
        CALL SAXPY (LENGAB,ONE,ICORE(I000),1,ICORE(I0GAB),1)
