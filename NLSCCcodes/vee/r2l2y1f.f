      SUBROUTINE R2L2Y1F(Y1,ICORE,MAXCOR,IUHF)
C
C Y1(ai) = 1/2 R(mn,ef)*W(gn,ef)*L(im,ag)
C
C        = L(im,ag) * Q(gm)
C
C WHERE Q(gm) = R(mn,ef)*W(gn,ef)
C
CEND
...
C
C CALCULATE R(mn,ef)*W(gn,ef) PART AND PUT ON LIST 490
C
      IOFFG=I0G
...
10    CONTINUE
      CALL DT2INT1A(ICORE(I0G),MAXCOR,IUHF,IRREPX,27,461,490)
      IOFFG=I0G
...
C
C   Y(ai) = [2 L(Im,Ag) - L(Mi,Ag)]*G(gm)
C
C FORM SPIN ADAPTED 2*L(Im,Ag)-L(Mi,Ag) AND PUT IN L(AI,gm) MATRIX
C
       LISTL1=437
       LISTL2=439
...
C
C EVALUTE PRODUCT Y(AI) = L(AI,gm)*Q(gm)
C
