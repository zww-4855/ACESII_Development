      SUBROUTINE H4R2ALL(ICORE,MAXCOR,ISTART,
     &                   IOFFT1,IOFFR1,ISPIN,IUHF)
C
...
C
C  AB             AE           Ae                
C Q  = Q(AAAA) = R  H4(EMJB) + R  H4(meJB)
C  IJ             IM           Im                   
C
C  ab             ae           aE                
C Q  = Q(BBBB) = R  H4(mejb) + R  H4(MEjb)
C  ij             im           iM                   
C
C  Ab             AE           Ae                
C Q  = Q(ABAB) = R  H4(MEjb) + R  H4(mejb)
C  Ij             IM           Im                   
C
C  aB             ae           aE
C Q  = Q(BABA) = R  H4(meJB) + R  H4(MEJB)
C  iJ             im           iM
C
C  Ab               eA
C Q  = Q(BAAB) = - R  H4(MeJb)
C  iJ               iM      
C
C  aB               Ae
C Q  = Q(ABBA) = - R  H4(mEjB)
C  Ij               Im
C
...
C THE FACTOR OF HALF IS TAKEN CARE OF IN HRRNGDRV  
C
CEND 
C
...
C 
      ...
      INTEGER POP,VRT
      CHARACTER*4 SPCASE
      DIMENSION ICORE(MAXCOR),IOFFT1(2),IOFFR1(2)
...
C
C               AB         EA
C   SOLVE FOR  Q   = SUM  R   *[- H4(MEJB)]  [FIRST PART OF Q(AAAA)]
C               IJ   M,E   IM
C
       MAXSIZ=0
       LISTR=453+ISPIN
       LISTW=413+ISPIN
       CALL HRRNGDRV(ICORE(ISTART),MXCOR,LISTW,LISTR,
     &               'TxW',MAXSIZ,INCREM,39+ISPIN,.TRUE.,
     &               ICORE(IOFFT1(ISPIN)),ICORE(IOFFT1(ISPIN)),
     &               ICORE(IOFFR1(ISPIN)),ICORE(IOFFR1(ISPIN)),
...
C
C               Ab           EA
C   SOLVE FOR  Q   =  SUM  R    H(MEjb)  [PART 1 OF Q(ABAB)]
C               Ij     M,E   IM
C
        LISTR=454
        LISTW=416
     ...
     &                ICORE(IOFFR1(1)),ICORE(IOFFR1(1)),      
     &                POP(1,1),POP(1,1),VRT(1,1),VRT(1,1),
     &                TWO,'AAAA',IUHF)
C
C               Ab         Ae
C   SOLVE FOR  Q   = SUM  R   *[-H(mejb)] [PART 2 OF Q(ABAB)]
C               Ij   m,e   Im
C
...
C
C               aB         aE
C   SOLVE FOR  Q   = SUM  R   *[-H(MEJB)] [PART 1 OF Q(BABA)].
C               iJ   M,E   iM
C
...
C
     ...
     &               ICORE(IOFFR1(2)),ICORE(IOFFR1(2)),
     &               POP(1,2),POP(1,2),VRT(1,2),VRT(1,2),
     &               TWO,'BBBB',IUHF)
C
C               aB           ea
C   SOLVE FOR  Q   =   SUM  R    H(meJB) [PART 2 OF Q(BABA)].
C               iJ     m,e   im
C
C NEED TO CHANGE LISTW FOR W(mbej) INTERMEDIATE PICKUP!
C
...
C
C               Ab         eA
C   SOLVE FOR  Q   = SUM  R   *[- H(MeJb)]  [Q(BAAB)]
C               iJ   M,e   iM
C
       LISTW=418+IUHF
       LISTR=459
...
C
C               aB         Ea
C   SOLVE FOR  Q   = SUM  R    *[-H(mEjB]  [Q(ABBA)]
C               Ij   m,E   Im
C
...
C
       ...
       I030=I020+ISCRSZ
       IF(I030.GT.MAXCOR)CALL INSMEM('H4R2ALL',I030,MAXCOR)
