...
C
        DO 100 IRREPE=1,NIRREP
         ...
         IOFFT=I0T(ISPIN)+(ISYMOFF(IRREPJ,1,8+ISPIN)-1)*IINTFP
         IOFFX=I010+(ISYMOFF(IRREPJ,IRREPR,20+ISPIN)-1)*DISSYL*IINTFP
         CALL XGEMM('N','N',NROW,NCOL,NSUM,ONEM,ICORE(IOFFL),NROW,
     &              ICORE(IOFFT),NSUM,ZILCH,ICORE(IOFFX),NROW)
100     CONTINUE
C
C NOW TRANSPOSE X(AI,MJ) -> X(AI,JM)
C
        CALL SYMTR1(IRREPR,POP(1,ISPIN),POP(1,ISPIN),DISSYL,
     &              ICORE(I010),ICORE(ITMP1),ICORE(ITMP2),ICORE(ITMP3))
C
C FORM X(AI,JM)*R(BM) => G(AI,JB)
C
        DO 200 IRREPM=1,NIRREP
         IRREPB=DIRPRD(IRREPM,IRREPX)
         ...
         IOFFX=I010+(ISYMOFF(IRREPM,IRREPR,20+ISPIN)-1)*DISSYL*IINTFP
         CALL XGEMM('N','T',NROW,NCOL,NSUM,ONE,ICORE(IOFFX),NROW,
     &              ICORE(IOFFR),NCOL,ZILCH,ICORE(IOFFG),NROW)
200     CONTINUE
C
...
c        call sscal(numdsg*dissyg,0.5d0,icore(i000),1)
...
C
        DO 300 IRREPE=1,NIRREP
         ...
         IOFFT=I0T(3-ISPIN)+(ISYMOFF(IRREPJ,1,11-ISPIN)-1)*IINTFP
         IOFFX=I010+(ISYMOFF(IRREPJ,IRREPR,23-ISPIN)-1)*DISSYL*IINTFP
...
C NOW TRANSPOSE X(AI,mj) -> X(AI,jm)
C
        CALL SYMTR1(IRREPR,POP(1,3-ISPIN),POP(1,3-ISPIN),DISSYL,
     &              ICORE(I010),ICORE(ITMP1),ICORE(ITMP2),ICORE(ITMP3))
C
C FORM X(AI,jm)*R(bm) => G(AI,jb)
C
        DO 400 IRREPM=1,NIRREP
         ...
         IOFFR=I0R(3-ISPIN)+(ISYMOFF(IRREPM,IRREPX,11-ISPIN)-1)*IINTFP
         IOFFX=I010+(ISYMOFF(IRREPM,IRREPR,23-ISPIN)-1)*DISSYL*IINTFP
...
C
        DO 500 IRREPE=1,NIRREP
         ...
         IOFFT=I0T(3-ISPIN)+(ISYMOFF(IRREPJ,1,11-ISPIN)-1)*IINTFP
         IOFFX=I010+(ISYMOFF(IRREPJ,IRREPR,ITX)-1)*DISSYL*IINTFP
         CALL XGEMM('N','N',NROW,NCOL,NSUM,ONE,ICORE(IOFFL),NROW,
     &              ICORE(IOFFT),NSUM,ZILCH,ICORE(IOFFX),NROW)
500     CONTINUE
C
...
C             X(aI,mJ) -> X(aI,Jm)
C
        CALL SYMTR1(IRREPR,POP(1,ISPIN),POP(1,3-ISPIN),DISSYL,
     &              ICORE(I010),ICORE(ITMP1),ICORE(ITMP2),ICORE(ITMP3))
C
C FORM 
C
C             X(Ai,jM)*R(BM) => G(Ai,jB)
C             X(aI,Jm)*R(bm) => G(aI,Jb)
C
        DO 600 IRREPM=1,NIRREP
         ...
         IOFFR=I0R(ISPIN)+(ISYMOFF(IRREPM,IRREPX,8+ISPIN)-1)*IINTFP
         IOFFX=I010+(ISYMOFF(IRREPM,IRREPR,ITX)-1)*DISSYL*IINTFP
         CALL XGEMM('N','T',NROW,NCOL,NSUM,ONEM,ICORE(IOFFX),NROW,
     &              ICORE(IOFFR),NCOL,ZILCH,ICORE(IOFFG),NROW)
600     CONTINUE
C
