...
CEND
      ...
      INTEGER POP,VRT,DIRPRD,CORE_WINDOW
      LOGICAL CORE_SEARCH
      CHARACTER*4 SPTYPE
...
C
      READ(30,*)NROOTS
      IF (CORE_SEARCH .AND. NROOTS .GT. 20) THEN
         Write(6,*) 
         Write(6,"(A,A)") " More than 20 core EEs are not allowed",
...
500     READ(30,*,END=991)ISPIN,INDEXI,INDEXJ,INDEXA,INDEXB,FACT
      ...
      IF (CORE_SEARCH .AND. NCONT .GT. 1) THEN
         Write(6,*)
         Write(6,"(a,a)") " Specify only a one dominant core",
     &                    " excitation since code is not currently"
         Write(6,"(a)")   " setup to work with more than one."
         call ERREX
      ENDIF
...
C
         ...
         IF (SPTYPE .EQ. ' AA ') THEN
            IF (CORE_WINDOW .GT. 0) THEN
               IF (INDEXI .LE. CORE_WINDOW) IORB_A(III) = INDEXI
            ELSE
               IORB_A(III) = INDEXI
            ENDIF 
         ELSE IF (SPTYPE .EQ. ' BB ') THEN
            IF (CORE_WINDOW .GT. 0) THEN
               IF (INDEXI .LE. CORE_WINDOW) JORB_B(III) = INDEXJ
            ELSE
               JORB_B(III) = INDEXI
            ENDIF 
         ENDIF 
...
C
         ...
         IF (SPTYPE .EQ. 'AAAA') THEN
	        ...
	        IF (INDEXI .LE. CORE_WINDOW) IORB_AA(III)= INDEXI
                IF (INDEXJ .LE. CORE_WINDOW) JORB_AA(III)= INDEXJ
             ELSE 
                IORB_AA(III)= INDEXI 
                JORB_AA(III)= INDEXJ
             ENDIF
         ELSE IF (SPTYPE .EQ. 'BBBB') THEN
             IF (CORE_WINDOW .NE.0) THEN
	        IF (INDEXI .LE. CORE_WINDOW) IORB_BB(III)= INDEXI
                IF (INDEXJ .LE. CORE_WINDOW) JORB_BB(III)= INDEXJ
             ELSE 
                IORB_BB(III)= INDEXI
                JORB_BB(III)= INDEXJ
             ENDIF 
         ELSE IF (SPTYPE .EQ. 'ABAB') THEN
             IF (CORE_WINDOW .NE.0) THEN
	        IF (INDEXI .LE. CORE_WINDOW) IORB_AB(III)= INDEXI
                IF (INDEXJ .LE. CORE_WINDOW) JORB_AB(III)= INDEXJ
             ELSE
                IORB_AB(III)= INDEXI
                JORB_AB(III)= INDEXJ
             ENDIF 
