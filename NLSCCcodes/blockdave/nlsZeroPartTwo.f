        ...
        subroutine NLSZEROPARTTWO(SCR,MAXCOR,IRREPX,IUHF,IROOT,NSIZE)
C
...
C
      ...
      INTEGER LS1OUT,Rdim,CISdim,EOMdim
...
!#endif
        CISdim=nocc*nvirt+nocc*nvirt*IUHF
        EOMdim=(nocc*nvirt)**2 + (nocc*nvirt*IUHF)**2
        Rdim=CISdim+EOMdim
        print*,'rdim,nsize'
        print*,Rdim,Nsize
        if (Rdim.ne.Nsize) then
          print*,'Riajb size not equal to Nsize'
!          stop
        endif
...
!     pre-dumps the R vector from block Davidson so that existing
!     code should run
      call LANCZOS_DUMP_VEC(irrepx,scr,maxcor,490,0,0,
     &                      443,0,IUHF,.False.)
...
C
! **PERFORM THE NLS-EOM PART NOW THAT SCR IS LOADED/DUMPEED TO DISK
      OLDSCR=SCR(1:NSIZEC)
      Call Hbarxc(SCR,Maxcor,Iuhf,1,Irrepx)
      Call Loadvec1(Irrepx,SCR,Maxcor,Iuhf,490,2,460,
     +                      NSIZEC,.False.)

!     oldscr^t * scr === R^t H R === E
      print*,'NLS-EOM-CCSD energy:', dot_product(OLDSCR,SCR(1:NSIZE))
