      ...
      SUBROUTINE LANCZOS_LOAD_VEC(IRREPX,SCR,MAXCOR,LIST1,IOFF1,
     &                            LIST2,IOFF2,IUHF,SPINAD)
C
...
C
      ...
      I0C1A=1
      CALL GETLST(SCR(I0C1A),1,1,1,1,LIST1+IOFF1)
      Write(6,*)
      Write(6,"(a,i3)") "@-tdee_load_vec"
      Write(6,"(a,i4)") "The target singles list",LIST1+IOFF1
      call checksum("AA  :",SCR(I0C1A),LENC1A,S)
      IF(IUHF.NE.0)THEN
       ...
       ITOP  =I0C2AA+LENC2AA
       CALL GETLST(SCR(I0C1B),1,1,1,2,LIST1+IOFF1)
       CALL GETALL(SCR(I0C2AA),LENC2AA,IRREPX,LIST2+1+IOFF2)
       CALL GETALL(SCR(I0C2BB),LENC2BB,IRREPX,LIST2+2+IOFF2)
      Write(6,"(a,2i4)") "The target doubles list",LIST2+1+IOFF2,
     +                    LIST2+2+IOFF2
      call checksum("BB  :",SCR(I0C1B),LENC1B,S)
      call checksum("AAAA:",SCR(I0C2AA),LENC2AA,S)
      call checksum("BBBB:",SCR(I0C2BB),LENC2BB,S)
      ELSE
       I0C2AB=I0C1A+LENC1A
       ITOP  =I0C2AB+LENC2AB
      ENDIF
      CALL GETALL (SCR(I0C2AB),LENC2AB,IRREPX,LIST2+3+IOFF2)
      Write(6,"(a,i4)") "The target doubles list", LIST2+3+IOFF2
      call checksum("ABAB:",SCR(I0C2AB),LENC2AB,S)
C
      IF (SPINAD) THEN
       IOFF=I0C2AB
       ITMP1=ITOP
