subroutine buildRSPert(V,D,p,q,Nsize,pVq,qVq,tmpHold,k,E_k,totE)
  integer,intent(in)::k,Nsize
  double precision,intent(in)::p(NSize,1),q(NSize,NSize-1),totE(1,1)
...
!  double precision::tmpHold(NSize-1,Nsize-1)
  ...
  if (k.eq.0) then
    ...
    tmpV=V
    do i=1,NSize
      tmpV(i,i)=tmpV(i,i)-totE(1,1)
    enddo
    qVq=matmul(transpose(q(:,:)),matmul(tmpV,q))
