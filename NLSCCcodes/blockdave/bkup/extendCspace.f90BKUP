      ...
      double precision:: Scr(NSize),HbarScr(Nsize)
      double precision:: q1(Nsize),r1(Nsize),factor
      integer::j,k
      double precision::r1norm,HbarDiag(NSize)
...
! Tvec - Eigenvector matrix of the tridiagonal T matrix, of dimension (i x i)
      ...
      CALL SCOPY(NSize,Hbar,NSize+1,HbarDiag,1) ! stores diagonal of CISmat
      HbarScr=0.0d0
      ...
      do j=1,initVec
         ...
         !HbarScr=Hbar
         r1=0.0d0
!****GOAL: Compute r1=(Hbar*C - eig*C)*Tvec
...
!   call dgemm('N','N',NSize,1,i,1.0d0,C(:,1:i),NSize,Tvec(:,j),i,Scr,NSize)
         ...
         cEVECS(:,j)=Scr
        !SCR=-thetaNew(j)*SCR
        HbarSCR=matmul(Hbar,Scr)
        r1=HbarSCR-thetaNew(j)*SCR
        call vecdiv2(thetaNew(j),r1,HbarDiag,r1,NSize)
        r1norm=dot_product(r1,r1)
        r1norm=dsqrt(r1norm)
        call dscal(Nsize,1.0d0/r1norm,r1,1)
