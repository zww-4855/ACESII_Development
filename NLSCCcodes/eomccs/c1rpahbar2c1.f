      ...
      SUBROUTINE C1RPAHBAR2C1(ICORE,MAXCOR,IUHF,IRREPX,WSPIN,
     &                        LISTW0,LISTT1,ISIDE,IMULT)
C
...
C
       IF(IUHF.EQ.0)THEN
        IF (IMULT .EQ. 1) LISTW=LISTW0-1+3
        IF (IMULT .EQ. 2) LISTW=LISTW0
       ELSE
        IF(ISIDE.EQ.1)THEN
...
C
       ...
       ENDIF
        Write(6,*) "LISTC1",LISTT1
        call checksum("@-c1rpaintc1:",ICORE(I010),LENTAR,s)
       ...
       ELSE
        ...
        IF (IMULT .EQ. 1) THEN 
            LISTWA=LISTW0-1+3
            CALL GETLST(ICORE(I020),1,NUMDSW,1,IRREPX,LISTWA)
            CALL DSCAL(NUMDSW*DISSYW,-ONE,ICORE(I020),1) 
...
C
     ...
     &              1,ICORE(I020),DISSYW,ONE,ICORE(I000),1)
       ENDIF
        call checksum("@-c1rpaintc1:",ICORE(I000),LENTAR,s)
C
