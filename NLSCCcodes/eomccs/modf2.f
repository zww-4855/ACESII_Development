      ...
      SUBROUTINE MODF2(ICORE,MAXCOR,IUHF,ITYPE)
C
...
C
      DO 5 ISPIN=1,1+IUHF
       CALL UPDMOI(1,NFMI(ISPIN),ISPIN+6,91,0,0)
       CALL UPDMOI(1,NFEA(ISPIN),ISPIN+6,92,0,0)
5     CONTINUE
...
C
      DO 10 ISPIN=1,1+IUHF
       ...
       IOFFEA=I020 
C
CSSS       CALL GETLST(ICORE(IOFFMI),1,1,1,2+ISPIN,91)
CSSS       CALL GETLST(ICORE(IOFFEA),1,1,1,2+ISPIN,92)
...
20     CONTINUE
       CALL PUTLST(ICORE(I010),1,1,1,ISPIN+6,91)
       CALL PUTLST(ICORE(I020),1,1,1,ISPIN+6,92)
10    CONTINUE
C
      RETURN
      END
